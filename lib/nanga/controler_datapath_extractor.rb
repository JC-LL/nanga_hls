module Nanga
  class ControlerDatapathExtractor <  CompilerPass
    def run ast
      ast.accept(self)
    end
    ...
    def visitDef func,args=nil
      ...
      csteps_h.each do |cstep,nodes|
        ...
        puts "cstep #{cstep} / state #{wr_state.id}".center(50,'=')
        nodes.each do |node|
          puts node.stmt.str.ljust(30,'.')+node.mapping.name
